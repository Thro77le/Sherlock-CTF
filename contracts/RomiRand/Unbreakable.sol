...
contract Unbreakable is ERC20, Ownable {
    ...
    function Error(string memory _error) external onlyBroken onlyPrivate {
        // something broke, try to fix it

        // success need to be false
        (bool success, bytes memory data) = address(this).call(bytes(_error)); // call to ?
        if (!success) {
            // try again
            (success, data) = address(this).call(data); // call to Panic
            bool isfixed = abi.decode(data, (bool)); // @audit-info - Need to return false

            // @audit-info - Need to set this to true
    ...
    // @audit-info - Pass require and selfdestruct in delegatecall
    // solved need to be msg.sender
