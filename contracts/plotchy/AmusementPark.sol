...
contract AmusementPark {
    ...
    */
    ...
    // blueCar == bytes("blue")
    // redCar = uint40(bytes5(bytes.concat("255", "0", "0")))
    function _BumperCars(bytes calldata blueCar, uint40 redCar, bytes calldata yellowCar) external insidePark {

        bytes memory nextRide;

        require(keccak256(blueCar) == keccak256("blue"), "blue error");
        require(redCar == uint40(bytes5(bytes.concat("255", "0", "0"))), "red error");
        bytes memory crash = abi.encodePacked(blueCar, redCar, yellowCar);

        (uint x , bytes memory _rainbowCar) = abi.decode(crash, (uint256, bytes));
        nextRide = abi.encodePacked(_rainbowCar);

        BumperCars = true;
        address(this).call(nextRide); // here call
        BumperCars = false;
    }
    ...
    function _Rollercoaster(bytes[] memory ticket) external insidePark {
        ...
        for (uint256 count = 0; count < 8; count++) {
            ...
            require(uint256(rideFeatures[count]) == singleCart, "cart error");
            cartChain >>= 3;
        }
    ...
    // houseLayout[0] =
    // houseLayout[0] =
    // houseLayout[3] = abi.encodeWithSelector("", arg);
    function _HouseOfMirrors(bytes[] calldata houseLayout) external insidePark {
        ...
        HouseOfMirrors = true;
        address(this).call(nextRide); // here call _leavePark
        MIRR0R = 0;
        MlRROR = 0;
        MlRR0R = 0;
    ...
    // @auid-info - must be insider call and need to ride all things
