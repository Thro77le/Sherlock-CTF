...
contract Superfluid is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    bytes32 public constant UPDATE_DETAILS_HASH = keccak256("UpdateStreamDetails(uint256 streamId,uint256 paymentPerBlock,uint256 startBlock,uint256 stopBlock,uint256 nonce)");
    ...
    function refuel(uint256 id, uint256 amount) public payable {
        if (getStream[id].sender != msg.sender) {
            revert Unauthorized();
        }
    ...
    function withdraw(uint256 id) public payable {
        if (getStream[id].recipient != msg.sender) {
            revert Unauthorized();
        }
    ...
    function calculateBlockDelta(Timeframe memory timeframe) internal view returns (uint256) {
        if (block.number <= timeframe.startBlock) {
            return 0;
        }
