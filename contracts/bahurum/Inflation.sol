...
contract InflationaryToken is Context, ERC20, Ownable {
    uint private _rate; // = 10
    bool private _goingBrr; // = true

    constructor(string memory name_, string memory symbol_, uint rate_, uint initialSupply) ERC20( name_, symbol_){
        _goingBrr = true;
        _rate = rate_;
        _print(initialSupply); // mint 1000 to owner
    }
    ...
    // beofre each token transfer, mints 10% to owner
