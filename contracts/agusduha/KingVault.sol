...
contract KingVault is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    uint256 private _lastWithdrawalTimestamp; // potentialy overwrittable
    address private _sweeper; // potentialy overwrittable

    modifier onlySweeper() {
        require(msg.sender == _sweeper, "Caller must be the king");
        _;
    }
    ...
    function withdrawAllFunds(address payable recipient) external onlySweeper {
...
contract GovernanceTimelock is AccessControl {
    ...
    constructor(address admin, address proposer) {
        ...
        _setupRole(ADMIN_ROLE, admin); // 0xDA0
        _setupRole(ADMIN_ROLE, address(this));

        _setupRole(PROPOSER_ROLE, proposer); // 0xDA2
    }
